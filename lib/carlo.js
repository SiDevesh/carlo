  ...
  async init_() {
    ...
    await Promise.all([
      ...
      this.browser_.defaultBrowserContext().
          overridePermissions(this.options_.host, [
            'geolocation',
            'midi',
            'notifications',
            'camera',
...
class Window extends EventEmitter {
  ...
  async load(uri = '', ...params) {
    ...
    await this.page_.goto(new URL(this.loadURI_, this.options_.host).toString(), {timeout: 0, waitFor: 'domcontentloaded'});
  ...
  async handleRequest_(request) {
    ...
    console.log('host received vs options value:', url.hostname, this.options_.host);
    if (url.hostname !== this.options_.host) {
    ...
    for (const {prefix, folder, baseURL} of this.app_.www_.concat(this.www_)) {
      ...
      console.log('filename as compination folder and pathname:' fileName);
