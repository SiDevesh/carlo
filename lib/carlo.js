  ...
  async init_() {
    ...
    await Promise.all([
      ...
      this.browser_.defaultBrowserContext().
          overridePermissions(new URL(this.options_.url).origin, [
            'geolocation',
            'midi',
            'notifications',
            'camera',
...
class Window extends EventEmitter {
  ...
  async load(uri = '', ...params) {
    ...
    await this.initializeNetworkInterception_();
    ...
    await this.page_.goto(new URL(this.loadURI_, `${new URL(this.options_.url).origin}/`).toString(), {timeout: 0, waitFor: 'domcontentloaded'});
  ...
  async handleRequest_(request) {
    ...
    console.log('host received vs options value:', url.hostname, new URL(this.options_.url).hostname);
    if (url.hostname !== new URL(this.options_.url).hostname) {
      request.deferToBrowser();
      return;
    }

    const urlpathname = url.pathname;
    console.log('urlpathname, gonna iterate nwah:', urlpathname);
    for (const {prefix, folder, baseURL} of this.app_.www_.concat(this.www_)) {
      debugServer('prefix:', prefix);
      console.log('prefix, folder, baseUrl:', prefix, folder, baseURL.toString());
      if (!urlpathname.startsWith(prefix)) {
        console.log('urlpathname starts with prefix: continue the loop');
        continue;
      }

      const pathname = urlpathname.substr(prefix.length);
      debugServer('pathname:', pathname);
      console.log('urlpathname substr by prefix.length:', pathname);
      if (baseURL) {
        console.log('baseUrl exists, return from function after defer');
        request.deferToBrowser({ url: String(new URL(pathname, baseURL)) });
        return;
      }
      const fileName = path.join(folder, pathname);
      console.log('filename as combination folder and pathname:', fileName);
      if (!fs.existsSync(fileName)) {
        console.log('filename doesnt exists, continue the loop');
        continue;
      }

      console.log('filename exists, read, fullfill and return');
      const headers = { 'content-type': contentType(request, fileName) };
      const body = await fsReadFile(fileName);
      request.fulfill({ headers, body});
      return;
    }
    console.log('never returned from loop and loop now oover, defer now');
    request.deferToBrowser();
  }
